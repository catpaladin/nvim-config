...
Default settings are in `lua/config.lua`. To customize without affecting git, create `lua/config.local.lua` (gitignored):
...
Settings are deep-merged, so you only need to specify overrides. After changing, restart Neovim and run `:Lazy sync`.
